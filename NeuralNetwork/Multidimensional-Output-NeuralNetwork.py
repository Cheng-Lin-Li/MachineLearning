'''
Machine Learning Algorithm Name: Neural Network with multidimensional output
...
@updated:   May, 15, 2017
    ...
    A Neural Network class supports on both logistic sigmoid and hyperbolic tangent functions as activation functions, resolver is stochastic gradient descent to implement and provide a lot of value sets for reference.
        1. This implementation apply Stochastic learning approach where single example is chosen from training set at each iteration.
...
if __name__ == '__main__':
    ...
    with open(TRAINING_SET) as f:
        for training_image in f.readlines():
            ...
            elif 'hold' in training_image:
                labels.append([0, 0, 1, 0])
            elif 'stop' in training_image:
                labels.append([0, 1, 0, 0])
            elif 'up' in training_image:
...
#                 labels.append([0,])      
    ...
    with open(TESTING_SET) as f:
        ...
        for test_image in f.readlines():
            ...
            if np.all(p == [0,0,0,1]) and ('down' in test_image):
                    print('Match(O)=>{}: Predict=True, Output value={}'.format(test_image, p))
                    correct += 1
            elif np.all(p == [0,0,1,0]) and ('hold' in test_image): 
                    print('Match(O)=>{}: Predict=True, Output value={}'.format(test_image, p))
                    correct += 1
            elif np.all(p == [0,1,0,0]) and ('stop' in test_image): 
                    print('Match(O)=>{}: Predict=True, Output value={}'.format(test_image, p))
                    correct += 1
            elif np.all(p == [1,0,0,0]) and ('up' in test_image): 
                    print('Match(O)=>{}: Predict=True, Output value={}'.format(test_image, p))
                    correct += 1
            else :
